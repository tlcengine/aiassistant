...
VOICEBOX_BASE = "http://127.0.0.1:17493"  # Local on geo2
...
async def text_to_speech(text: str, profile_id: str = KRISHNA_PROFILE_ID) -> bytes:
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        ...
        audio_resp = await client.get(f"{VOICEBOX_BASE}/audio/{data['id']}")
...
async def list_profiles() -> list:
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        resp = await client.get(f"{VOICEBOX_BASE}/profiles")
        resp.raise_for_status()
        return resp.json()
